...
function PointerEventController({ cardRef, dragged }) {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e) => {
      ...
      // Project only the card center
      const cardV = new THREE.Vector3(cardPos.x, cardPos.y - 1.2, cardPos.z);
      cardV.project(camera);

      const rect = canvasEl.getBoundingClientRect();
      const cardX = rect.left + (cardV.x * 0.5 + 0.5) * rect.width;
      const cardY = rect.top + (cardV.y * -0.5 + 0.5) * rect.height;

      const dx = e.clientX - cardX;
      const dy = e.clientY - cardY;
      const dist = Math.sqrt(dx * dx + dy * dy);

      // Within 180px of the card center, enable pointer events
      if (dist < 180) {
        canvasEl.style.pointerEvents = 'auto';
      } else {
...
function Band({
  ...
  return (
    <>
      <PointerEventController cardRef={card} dragged={dragged} />
